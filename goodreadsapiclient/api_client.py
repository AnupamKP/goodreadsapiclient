...
class GoodreadsAPIClient:
    ...
    def get_book_details(self, book_url: str) -> dict:
        ...
        try:
            ...
            api_response_json = xmltodict.parse(api_response.text)["GoodreadsResponse"][
                "book"
            ]
            response = {}

            response["title"] = (
                str(api_response_json["title"]) if api_response_json["title"] else None
            )
            response["average_rating"] = (
                float(api_response_json["average_rating"])
                if api_response_json["average_rating"]
                else None
            )
            response["ratings_count"] = (
                int(api_response_json["ratings_count"])
                if api_response_json["ratings_count"]
                else None
            )
            response["num_pages"] = (
                int(api_response_json["num_pages"])
                if api_response_json["num_pages"]
                else None
            )
            response["image_url"] = (
                str(api_response_json["image_url"])
                if api_response_json["image_url"]
                else None
            )
            response["publication_year"] = (
                str(api_response_json["publication_year"])
                if api_response_json["publication_year"]
                else None
            )
